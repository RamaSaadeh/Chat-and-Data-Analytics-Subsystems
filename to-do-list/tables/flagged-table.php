...
<table class="table table-borderless">
  <tbody>
    ...
    $sql = "SELECT * FROM todoList WHERE todoOwnerID = $todoOwnderID AND todoCompletion = 0 AND todoFlag = 1;";
    ...
    ?>
      ...
      <tr id="flagged-row-<?php echo $row["todoID"] ?>" data-bs-toggle="modal" data-bs-target="#viewModal-<?php echo $row["todoID"] ?>">
        <!-- script for mouse hovering -->
        <script>
          //mouse entering
          $("#flagged-row-<?php echo $row["todoID"] ?>").mouseenter(function() {
            document.body.style.cursor = "pointer";
            document.getElementById("flagged-row-<?php echo $row["todoID"] ?>").style.backgroundColor = "#ECECEC";
          })
          //mouse leaving
          $("#flagged-row-<?php echo $row["todoID"] ?>").mouseleave(function() {
            document.body.style.cursor = "default";
            document.getElementById("flagged-row-<?php echo $row["todoID"] ?>").style.backgroundColor = "white";
          })
        </script>
        <!-- priority data -->
